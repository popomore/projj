...
class Command extends BaseCommand {
  ...
  forwardScriptOutput(stdio) {
    ...
    if (stdio.stderr) {
      for (const line of splitLines(stdio.stderr)) {
        this.childLogger.error(line);
      }
    }
  }
...
function normalizeProxyEnv(env) {
  ...
  applyProxyPair(normalized, 'http_proxy', 'HTTP_PROXY', pickFirstEnvValue(env, [
    'http_proxy',
    'HTTP_PROXY',
    'npm_config_proxy',
    'NPM_CONFIG_PROXY',
  ]));
  applyProxyPair(normalized, 'all_proxy', 'ALL_PROXY', pickFirstEnvValue(env, [
    'all_proxy',
    'ALL_PROXY',
  ]));
  applyProxyPair(normalized, 'no_proxy', 'NO_PROXY', pickFirstEnvValue(env, [
    'no_proxy',
    'NO_PROXY',
    'npm_config_noproxy',
    'NPM_CONFIG_NOPROXY',

...
describe('test/projj_find.test.js', () => {
  ...
  it('should support ignore_case with eggjs/egg', done => {
  ...
  it('should support ignore_case with eggjs', done => {
    const home = path.join(fixtures, 'find-ignore-case');
    mm(process.env, 'HOME', home);
    coffee.fork(binfile, [ 'find', 'eggjs' ])
    .expect('stdout', new RegExp(`find repo eggjs's location: ${home}/github.com/EGGJS/EGG`))
    .expect('code', 0)
    .end(done);
  });
